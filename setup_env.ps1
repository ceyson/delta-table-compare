...
& $VenvPython -m pip install --upgrade pip setuptools wheel -q --timeout 60

if ($Polars) {
    Write-Host "`nInstalling Polars-only dependencies (no PySpark)..." -ForegroundColor Cyan
    & $VenvPython -m pip install "pyarrow>=14.0" "polars>=1.0" "deltalake>=0.18" "pytest>=7.0" "pytest-timeout>=2.2" -q --timeout 60
} else {
    ...
    & $VenvPython -m pip install -r $ReqFile -q --timeout 120 --retries 3
}

# Install project editable
Write-Host "`nInstalling recon package (editable)..." -ForegroundColor Cyan
& $VenvPython -m pip install -e $ScriptDir -q --timeout 60
...
Write-Host ("=" * 60) -ForegroundColor Green
Write-Host ""
Write-Host "  Venv is ACTIVE in this terminal."
